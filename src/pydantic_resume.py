...
if __name__ == "__main__":
    ...
    json_resume = data.get("resume")
    
    # create the PydanticResume object from the json resume data
    try:
        pydantic_resume = PydanticResume(**json_resume)
        print("JSON resume is valid and the PydanticResume object has been created.")
    ...
    # Convert JSON resume object to string and replace characters
    json_resume_str = json.dumps(json_resume).\
        replace('"', "").replace(": ", ":").\
            replace("{","(").replace("}",")")
    ...
    if pydantic_resume_str == json_resume_str:
        ...
        print("The PydanticResume str does not match the JSON resume str")
        print("PydanticResume str:")
        print(pydantic_resume_str)
        print("JSON resume str:")
